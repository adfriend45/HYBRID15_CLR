...
!----------------------------------------------------------------------!
do kl = 1, nlayers
  swc (kl) = sm_day (kl) / (SM_max (kl) / swc_field_capacity)
end do
!next decomp by layer after Wbal
if (swc (1) > swc (2)) then
  wfps = 100.0 * swc (1) / denom
else
  wfps = 100.0 * swc (2) / denom
end if
...
!----------------------------------------------------------------------!
root_input  = total_input * root_fraction
shoot_input = total_input * (one - root_fraction)
!----------------------------------------------------------------------!
...
! Pool 3: active SOM.
!----------------------------------------------------------------------!
silt_plus_clay = silt_fraction + clay_fraction
